...
def _remove_colormap(filename):
  ...
  """
  plant = Image.open(filename)
  # Enforce 256 colors
  plant = plant.convert("P",dither=None,palette=Image.ADAPTIVE)
  width, height = plant.size

  planthandle = plant.load()
  for x in range(0, width):
      for y in range(0, height):
          if planthandle[x,y] == 1:
	  # Set ignore label to 255
              plant.putpixel((x, y), 255)
          if planthandle[x,y] in [2, 3]:
	  # Shift root and shoot labels by 1, to make sure label range fits the tensor's shape
              plant.putpixel((x,y), planthandle[x,y] - 1)

  return np.array(plant)
